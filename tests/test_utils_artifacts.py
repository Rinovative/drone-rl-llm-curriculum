...
def test_storage_root_respects_environment(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert (
        utils.artifacts.get_evaluation_run_dir("eval_ppo_hover_smoke_on_hover") == storage_root / "evaluation_runs" / "eval_ppo_hover_smoke_on_hover"
    )
    assert utils.artifacts.get_training_metrics_dir("ppo_tracking_smoke") == storage_root / "training_runs" / "ppo_tracking_smoke" / "metrics"
    assert (
        utils.artifacts.get_evaluation_renders_dir("eval_ppo_hover_smoke_on_hover")
        == storage_root / "evaluation_runs" / "eval_ppo_hover_smoke_on_hover" / "renders"
    )
    assert (
        utils.artifacts.get_evaluation_traces_dir("eval_ppo_hover_smoke_on_hover")
        == storage_root / "evaluation_runs" / "eval_ppo_hover_smoke_on_hover" / "traces"
    )
    assert utils.artifacts.get_training_models_dir("ppo_tracking_smoke") == storage_root / "training_runs" / "ppo_tracking_smoke" / "models"
    assert (
        utils.artifacts.get_evaluation_plots_dir("eval_ppo_hover_smoke_on_hover")
        == storage_root / "evaluation_runs" / "eval_ppo_hover_smoke_on_hover" / "plots"
    )
...
def test_ensure_category_run_dirs_create_new_layouts(tmp_path: Path) -> None:
    ...
    training_paths = utils.artifacts.ensure_training_run_dirs("ppo_hover_smoke", storage_root=tmp_path)
    evaluation_paths = utils.artifacts.ensure_evaluation_run_dirs("eval_ppo_hover_smoke_on_hover", storage_root=tmp_path)
    comparison_paths = utils.artifacts.ensure_comparison_report_dirs("comparison_smoke", storage_root=tmp_path)

    assert training_paths["run"] == tmp_path / "training_runs" / "ppo_hover_smoke"
    assert evaluation_paths["run"] == tmp_path / "evaluation_runs" / "eval_ppo_hover_smoke_on_hover"
