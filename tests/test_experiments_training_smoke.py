...
def test_training_smoke_config_loads_default_settings() -> None:
    ...
    settings = training_smoke.load_training_smoke_settings("tests/fixtures/configs/smoke/training_smoke.yaml")

    assert settings.task_config_path.as_posix() == "tests/fixtures/configs/smoke/trajectory_validation.yaml"
...
def test_cli_parser_builds_settings_without_running_training() -> None:
    ...
    args = parser.parse_args(["--config", "tests/fixtures/configs/smoke/training_smoke.yaml", "--max-steps", "3", "--task-index", "1"])
